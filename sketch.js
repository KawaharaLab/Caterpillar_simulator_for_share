...
//var PI_coef_1_array = [0.8];
//var PI_coef_1_array = [2.5, 3.0, 3.5, 4.0];
var PI_coef_1_array = [4/3*0.3, 4/3*0.4, 4/3*0.5, 4/3*0.6, 4/3*0.7, 4/3*0.8, 4/3*0.9, 4/3*1.0, 4/3*1.1, 4/3*1.2];

var PI_coef_re_center = 0;
var PI_coef_re_center_counter = 0;
//var PI_coef_re_center_array = [0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75];
var PI_coef_re_center_array = [0.5];

var PI_coef_re_width = 0.25;
var PI_coef_re_width_counter = 0;
//var PI_coef_re_width_array = [0];
//var PI_coef_re_width_array = [1.0];
//PI_coef_re_width_array = [0.25];
var PI_coef_re_width_array = [0.25];// 1/numMass?
//var PI_coef_re_width_array = [0.05, 0.1, 0.15, 0.2];
//var PI_coef_re_width_array = [0.2, 0.3, 0.35, 0.4, 0.45];
//var PI_coef_re_width_array = [0.55, 0.6, 0.65];
//var PI_coef_re_width_array = [1.8, 1.9, 0.1, 0.2];
  ...
  this.calc_f = function() {
    ...
    if (this.l != 0) {
      ...
      if(this.L_rts < this.l){
        ...
        this.force = dis.mult(-this.k);//if non extrudable, this.force.set(0, 0);
      }
    } else {
      this.force.set(0, 0);
    }
...
function mousePressed() {pressed = 1;}
function mouseDragged() {dragging = 1;}
function mouseReleased() {dragging = 0;pressed = 0;}
...
/*
  //ねじれによって体節に生じる力の計算と、その加算。
  for (var i = 1; i < numMass - 1; i++) {
    ts[i].calc_f();
    particle[i - 1].add_f(ts[i].force_from);
    particle[i].add_f(ts[i].force_mid);
    particle[i + 1].add_f(ts[i].force_to);
  }*/
  ...
  //マウスでドラッグして体節を動かせるように。
  if (dragging == 1) {
    var mouse_r = createVector(mouseX, mouseY);
//    text("mouse: ", 10, 40); text(mouse_r, 40, 40);
    for (var i = 0; i < numMass; i++) {
      var d = p5.Vector.dist(mouse_r, particle[i].r);
      if (d < radius) {
        particle[i].r.set(mouse_r.x, mouse_r.y);
      }
    }
  }
  //髭の描写
...
//      leg_left.set(particle[i].v);
  ...
  text("angle: ", 10, 180);
  text(round3disp(direction_End), 100, 180);
  text("realtime angle: ", 10, 240);
  text(round3disp(get_direction_deg()), 100, 240);
  ...
  //≈text("k is 3 time!■■■■■■■■■■", 200, 320);

  //text("PIcoef1 = minus!■■■■■■■■■■", 200, 320);
    /*text(particle[0].f,100,260);
    text(particle[1].f,100,280);
    text(gravity,100,300);
    text(sp[0].force,100,320);
*/

  if (time_step == MAX_TIME_STEP) {
    //save(time_list, 'time.data');//小数点があるときはsaveStrings
    //save(PI_coef_1_array, 'time.data');//小数点があるときはsaveStrings
    //saveTable(table, 'data.txt'); //これをエクセルで開けば，データが一列の表示されれる
  }
  time_step++;
}
